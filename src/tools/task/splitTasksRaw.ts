...
export const splitTasksRawSchema = z.object({
  ...
  tasksRaw: z
    .string()
    .describe(
      "结构化的任务清单，每个任务应保持原子性且有明确的完成标准，避免过于简单的任务，简单修改可与其他任务整合，避免任务过多，范例：[{name: '简洁明确的任务名称，应能清晰表达任务目的', description: '详细的任务描述，包含实施要点、技术细节和验收标准', implementationGuide: '此特定任务的具体实现方法和步骤，请参考之前的分析结果提供精简pseudocode', notes: '补充说明、特殊处理要求或实施建议（选填）', dependencies: ['此任务依赖的前置任务完整名称'], relatedFiles: [{path: '文件路径', type: '文件类型 (TO_MODIFY: 待修改, REFERENCE: 参考资料, CREATE: 待建立, DEPENDENCY: 依赖文件, OTHER: 其他)', description: '文件描述', lineStart: 1, lineEnd: 100}], verificationCriteria: '此特定任务的验证标准和检验方法'}, {name: '任务2', description: '任务2描述', implementationGuide: '任务2实现方法', notes: '补充说明、特殊处理要求或实施建议（选填）', dependencies: ['任务1'], relatedFiles: [{path: '文件路径', type: '文件类型 (TO_MODIFY: 待修改, REFERENCE: 参考资料, CREATE: 待建立, DEPENDENCY: 依赖文件, OTHER: 其他)', description: '文件描述', lineStart: 1, lineEnd: 100}], verificationCriteria: '此特定任务的验证标准和检验方法'}]"
    ),
      ...
      content: [
        {
          type: "text" as const,
          text:
            "tasksRaw 参数格式错误，请确保格式正确，请尝试修正错误，如果文本太长无法顺利修复请分批呼叫，这样可以避免讯息过长导致不好修正问题，错误讯息：" +
